...
import {editAboutText, editBannerText, getAboutText, getBannerText} from "../../../redux/admin-reduser";
...
const AdminHome: FC<PropsTypeAdminProducts> = ({setTitle}) => {

    const [text, setText] = useState('');
    const [changeText, setChangeText] = useState(false);
    const classes = useStyles();
    const dispatch = useDispatch();
    const aboutText = useSelector((state:RootState)=> state.admin.bannerText);
    ...
    const handleSubmitText = (e: React.SyntheticEvent, newText: string, id: string) => {
        e.preventDefault();
        dispatch(editBannerText(newText, id));
    };
